...
async fn backup_once(
    ...
    for task in tasks {
        ...
        if task.project_id > 0 && seen_projects.insert(task.project_id) {
            if let Some(project) = db
                ...
                .context("get project for backup")?
            {
                let uploads =
                    backup_project_uploads(db, storage, target, &project, include_uploads).await?;
    ...
    let notes = if include_uploads {
        vec!["Uploaded source files are included because backup_mode=include_uploads.".to_string()]
    } else {
        vec![
    ...
    let payload =
        serde_json::to_vec_pretty(&manifest).context("serialize active backup manifest")?;
...
async fn maybe_archive_session_dir(
    ...
    let archive_path =
        tokio::task::spawn_blocking(move || archive_directory(&session_path, "session"))
            .await
...
async fn maybe_backup_worktree_bundle(
    target: &BackupTarget,
    task: &Task,
) -> Result<Option<String>> {
    let task = task.clone();
...
async fn maybe_backup_untracked_files(
    target: &BackupTarget,
    task: &Task,
) -> Result<Option<String>> {
    let task = task.clone();
    ...
    let archive_path =
        tokio::task::spawn_blocking(move || create_untracked_archive(&task_for_archive))
            .await
