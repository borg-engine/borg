...
pub(crate) async fn get_task(
    ...
    match state
        .db
        .get_task_with_outputs_in_workspace(workspace.id, id)
        .map_err(internal)?
    {
...
pub(crate) async fn get_task_diagnostics(
    ...
    let task = require_task_access(state.as_ref(), &workspace, id)?;
...
pub(crate) async fn create_task(
    ...
    tracing::info!(
        ...
        requires_exhaustive_corpus_review = task.requires_exhaustive_corpus_review,
        title_len = task.title.chars().count() as u64,
        description_len = task.description.chars().count() as u64,
    );
...
pub(crate) async fn approve_task(
    ...
    let pid = (task.project_id > 0).then_some(task.project_id);
    let _ = state
        .db
...
pub(crate) async fn reject_task(
    ...
    tracing::info!(
        ...
        project_id = task.project_id,
        mode = task.mode.as_str(),
        feedback_len = reason.chars().count() as u64,
    );
...
pub(crate) async fn request_revision(
    ...
    tracing::info!(
        ...
        project_id = task.project_id,
        mode = task.mode.as_str(),
        target_phase = target_phase.as_str(),
        feedback_len = feedback.chars().count() as u64,
    );
...
pub(crate) async fn get_task_citations(
    ...
    let _task = require_task_access(state.as_ref(), &workspace, id)?;
...
pub(crate) async fn retry_all_failed(
    ...
    let tasks = state
        .db
        .list_all_tasks_in_workspace(workspace.id, None)
        .map_err(internal)?;
    let mut count = 0;
    for task in &tasks {
