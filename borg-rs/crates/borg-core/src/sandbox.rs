...
mod tests {
    ...
    fn bind_pos_of(args: &[String], target: &str) -> Option<usize> {
        args.windows(3)
            .enumerate()
            .find(|(_, w)| w[0] == "--bind" && w[1] == target && w[2] == target)
            .map(|(i, _)| i)
    }
    ...
    fn ro_bind_root_before_dev() {
        ...
        assert!(ro < dev, "--ro-bind / / must appear before --dev /dev");
    }

    #[test]
    fn dev_before_all_bind_entries() {
        let dir = make_test_dir("bwrap_test_dev_order");
        let dir_str = dir.to_str().unwrap().to_string();
        let args = Sandbox::bwrap_args(&[&dir_str], "/work", &cmd());
        let dev = pos(&args, "--dev").expect("--dev missing");
        for (i, arg) in args.iter().enumerate() {
            if arg == "--bind" {
                assert!(dev < i, "--dev must precede --bind at index {i}");
            }
        }
    ...
    #[test]
    fn existing_writable_dir_produces_bind() {
        let dir = make_test_dir("bwrap_test_existing_dir");
        let dir_str = dir.to_str().unwrap().to_string();
        let args = Sandbox::bwrap_args(&[&dir_str], "/work", &cmd());
        assert!(
            bind_pos_of(&args, &dir_str).is_some(),
            "existing dir should produce --bind {dir_str} {dir_str}"
        );
        let _ = fs::remove_dir(&dir);
    }

    #[test]
    fn nonexistent_dir_silently_omitted() {
        let bogus = "/nonexistent_bwrap_sandbox_test_xyz_12345";
        assert!(!Path::new(bogus).exists(), "test requires this path to not exist");
        let args = Sandbox::bwrap_args(&[bogus], "/work", &cmd());
        assert!(
            !args.iter().any(|a| a == bogus),
            "non-existent dir must not appear in args"
        );
    }

    #[test]
    fn tmp_bind_appears_after_writable_dirs() {
        let dir = make_test_dir("bwrap_test_tmp_order");
        let dir_str = dir.to_str().unwrap().to_string();
        let args = Sandbox::bwrap_args(&[&dir_str], "/work", &cmd());
        let dir_bind = bind_pos_of(&args, &dir_str).expect("existing dir should produce --bind");
        let tmp_bind = bind_pos_of(&args, "/tmp").expect("--bind /tmp /tmp always expected");
        assert!(
            dir_bind < tmp_bind,
            "--bind /tmp /tmp must follow per-dir binds; dir at {dir_bind}, /tmp at {tmp_bind}"
        );
        let _ = fs::remove_dir(&dir);
    }

    #[test]
    fn namespace_flags_in_order() {
        let args = Sandbox::bwrap_args(&[], "/work", &cmd());
        let unshare = pos(&args, "--unshare-pid").expect("--unshare-pid missing");
        let new_sess = pos(&args, "--new-session").expect("--new-session missing");
        let dwp = pos(&args, "--die-with-parent").expect("--die-with-parent missing");
        let proc_flag = pos(&args, "--proc").expect("--proc missing");
        assert_eq!(args[proc_flag + 1], "/proc", "--proc must be followed by /proc");
        assert!(unshare < new_sess, "--unshare-pid must precede --new-session");
        assert!(new_sess < dwp, "--new-session must precede --die-with-parent");
        assert!(dwp < proc_flag, "--die-with-parent must precede --proc");
    }

    #[test]
    fn namespace_flags_after_tmp_bind() {
        let args = Sandbox::bwrap_args(&[], "/work", &cmd());
        let tmp_bind = bind_pos_of(&args, "/tmp").expect("--bind /tmp /tmp missing");
        let unshare = pos(&args, "--unshare-pid").expect("--unshare-pid missing");
        assert!(
            tmp_bind < unshare,
            "--bind /tmp /tmp must precede --unshare-pid"
        );
    }

    #[test]
    fn chdir_and_command_after_separator() {
        let args = Sandbox::bwrap_args(&[], "/workdir", &cmd());
        let chdir = pos(&args, "--chdir").expect("--chdir missing");
        assert_eq!(args[chdir + 1], "/workdir");
        let sep = pos(&args, "--").expect("-- separator missing");
        assert!(chdir < sep, "--chdir must precede --");
        let rest = &args[sep + 1..];
        assert_eq!(rest, cmd().as_slice(), "command must follow -- separator");
    }

    #[test]
    fn no_writable_dirs_produces_one_bind() {
        let args = Sandbox::bwrap_args(&[], "/work", &cmd());
        let bind_count = args.iter().filter(|a| a.as_str() == "--bind").count();
        assert_eq!(
            bind_count, 1,
            "with no writable_dirs only the /tmp bind should appear"
        );
        assert!(
            bind_pos_of(&args, "/tmp").is_some(),
            "--bind /tmp /tmp should always be present"
        );
    }
}
