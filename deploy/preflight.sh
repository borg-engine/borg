...
token_json="$(curl -fsS "${BASE_URL}/api/auth/token" 2>/dev/null || true)"
token="$(printf '%s' "${token_json}" | jq -r '.token // empty' 2>/dev/null || true)"
if [[ -z "${token}" ]]; then
  warn "could not fetch auth token — skipping settings checks"
else
  ok "auth token acquired"

  settings="$(curl -fsS -H "Authorization: Bearer ${token}" "${BASE_URL}/api/settings" 2>/dev/null || true)"
  if [[ -z "${settings}" ]]; then
    warn "could not fetch /api/settings"
  else
    ok "settings endpoint reachable"
