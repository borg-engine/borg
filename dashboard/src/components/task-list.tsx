...
export function TaskList({ selectedId, onSelect, repoFilter }: TaskListProps) {
  ...
  const filtered = repoFilter ? tasks?.filter((t) => t.repo_path === repoFilter) : tasks;
  ...
  return (
    ...
    >
      ...
      {task.last_error && isActive && (
        <div className="pl-[32px] truncate text-[11px] text-red-400/60">{task.last_error}</div>
      )}
    </button>
  );
}
