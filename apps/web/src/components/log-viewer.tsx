...
export function LogViewer({ logs }: { logs: LogEvent[] }) {
  ...
  const rawFilteredLogs = useMemo(
    () => (viewMode === "live" ? (levelFilter === "all" ? logs : logs.filter((l) => l.level === levelFilter)) : []),
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div ref={containerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto overscroll-contain">
        <div className="p-3">
          {viewMode === "live" ? (
