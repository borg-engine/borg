    ...
    <div className={cn("flex gap-2.5", isUser ? "flex-row-reverse" : "flex-row")}>
      ...
      >
        <span
          className={cn(
            "text-[10px] font-bold",
            isUser ? "text-amber-200" : msg.role === "system" ? "text-zinc-500" : "text-amber-300",
          )}
        >
      ...
      >
        ...
        {isUser ? (
          ...
          <ChatMarkdown text={msg.content} variant="bubble" />
        )}
        {isPending && (
        ...
        {isUser && msg.delivered_phase && (
          <div className="mt-1.5 text-[10px] text-zinc-600">delivered at {msg.delivered_phase}</div>
        )}
      </div>
    </div>
  );
